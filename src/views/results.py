...
class VentanaProcesamiento:
    ...
    def _dibujar_grafico(self, precision):
        ...
        if self.data["tipo"] == "Discreta":
            ...
            intervalos = self.data["intervalos"]
            clases = [f"[ {i[0]} - {i[1]} >" for i in intervalos]
    ...
    def mostrar_resultados_estadisticos(self):
        ...
        self.tabla_estadistica = ttk.Treeview(
            self.frame_derecho, columns=("Medida", "Valor"), show="headings", height=15
        )
        ...
        medidas = [
            ...
            (
                "Moda",
                (
                    ", ".join(map(str, self.data["moda"]))
                    if isinstance(self.data["moda"], list)
                    else str(self.data["moda"])
                ),
            ),
